...
│  repair_app/bridge/      （通信桥接层 ★生产路径）│
│  repair_app/communication/（旧版通信, 已弃用）   │
│  repair_app/platform/    （操作系统抽象）       │
│  repair_app/export/      （G-code、PDF）      │
└─────────────────────────────────────────────┘
```

> **通信层迁移（已定稿）**：`repair_app/bridge/` 已取代 `repair_app/communication/` 成为生产通信路径。
> MainWindow → CoordinationService → LegacyZmqClient(bridge 适配器) → BridgeClient → ZMQ（端口 5555，Protobuf v2.1）。
> 旧版 `communication/zmq_client.py` 已标记为 deprecated，保留用于向后兼容。
> **TCP/JSON/Protocol v3.0/端口 5570 通信栈（`engine/matlab_engine.py`、`engine/factory.py`、`bridge/protocol_v3/`、`matlab_server/`）已全部弃用**，保留用于向后兼容与测试。
> 唯一生产通信路径与弃用说明详见 `docs/COMMUNICATION.md`。
