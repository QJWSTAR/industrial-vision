...
class MainWindow(QMainWindow):
    ...
    def _set_busy(self, busy: bool) -> None:
        ...
        if can_path and self._repair_mode == MODE_REPAIRING:
            sel_mask = self._selector.get_selection_mask() if self._selector else None
            can_path = sel_mask is not None and np.any(sel_mask)
    ...
    def _finish_path_planning(self, waypoints: np.ndarray, sel_mask: np.ndarray, show_dialog: bool = True) -> None:
    ...
    def _on_zmq_path_result(self, result_msg) -> None:
        ...
        try:
            ...
            self._finish_path_planning(waypoints[:, :3], sel_mask)
