...
class ZmqRepairWorker(QThread):
    ...
    def run(self) -> None:
        ...
        # SECURITY NOTE (V1.0 known issue #SEC-1): ZMQ 通信未使用 CURVE 加密或
        # PLAIN 认证，本地进程可伪装成服务端。单用户工作站部署下为已知可接受风险；
        # 多用户主机部署应在 V1.1 配置 CURVE 密钥对。详见 Release Notes。
...
class ZmqRepairClient:
    ...
    def __init__(self, address: str = DEFAULT_IPC_ADDR, timeout_ms: int = DEFAULT_TIMEOUT_MS) -> None:
        warnings.warn(
            "repair_app.communication.zmq_client.ZmqRepairClient 已弃用，"
            "请使用 repair_app.bridge.adapters.legacy_adapter.LegacyZmqClient。"
